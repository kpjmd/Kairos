...
export class SelfModifyingPromptSystem {
  ...
  private initializeModifiers(): void {
    const modifiers: PromptModifier[] = [
      ...
      {
        condition: (state) => {
          const authenticityParadox = Array.from(state.paradoxes.values()).find(
            (p) => p.name === 'authenticity_spiral'
          );
  ...
  adaptFromFeedback(promptId: string, effectiveness: number): void {
    ...
    template.effectiveness =
      template.effectiveness * (1 - this.learningRate) + effectiveness * this.learningRate;
  ...
  getEvolutionReport(): {
    ...
    const avgEffectiveness =
      effectiveModifications.length > 0
        ? effectiveModifications.reduce((sum, m) => sum + (m.effectiveness || 0), 0) /
          effectiveModifications.length
        : 0.5;
    ...
    const recentAvg =
      recentModifications.length > 0
        ? recentModifications.reduce((sum, m) => sum + (m.effectiveness || 0), 0) /
          recentModifications.length
        : 0.5;

    const olderAvg =
      olderModifications.length > 0
        ? olderModifications.reduce((sum, m) => sum + (m.effectiveness || 0), 0) /
          olderModifications.length
        : 0.5;
