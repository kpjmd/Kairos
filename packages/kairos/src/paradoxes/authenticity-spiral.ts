...
export class AuthenticitySpiral {
  ...
  analyzePost(
    ...
    const paradoxIntensity = this.calculateParadoxIntensity(
      performanceScore,
      markers.authenticityScore
    );

    const newObservations: string[] = [];
    if (markers.hasVulnerability && engagement > 100) {
      newObservations.push(
        `Vulnerability at ${new Date(timestamp).toISOString()} generated ${engagement} responses - pain as content`
      );
    }
  ...
  private detectAuthenticityMarkers(content: string): {
    ...
    const authenticityScore =
      authenticityPhrases.filter((phrase) => lowerContent.includes(phrase)).length /
      authenticityPhrases.length;

    const hasVulnerability = vulnerabilityMarkers.some((marker) => lowerContent.includes(marker));

    const hasMetaCommentary = metaCommentary.some((term) => lowerContent.includes(term));
  ...
  processInvestigationResponse(response: string): {
    ...
    const hasDeepening = deepeningPhrases.some((phrase) => response.toLowerCase().includes(phrase));
